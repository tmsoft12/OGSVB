...
func GetEventsByTopic(c *fiber.Ctx) error {
	...
	limit := c.QueryInt("limit", 10) // Varsayılan limit 10
	page := c.QueryInt("page", 1)    // Varsayılan sayfa 1

	// Sayfa başına kaç veri alınacağı ve hangi verilerden başlanacağı hesaplanır
	offset := (page - 1) * limit

	// Veritabanı sorgusunu pagination ile güncelle
